...
- **NEW site design** — full redesign of VaiBReport frontend (move beyond minima theme). Re-enable design skills (frontend-design, web-design-guidelines, stitch-\*) in `.claude/settings.local.json` when starting this work.
- **NEW custom URL** — acquire and configure custom domain for VaiBReport (replaces chaosaigent.github.io/VaiBReport/). Required before email delivery.
...
- **Email digest delivery** — depends on custom domain. Resend API key, sending domain, subscriber collection.
- **Platform migration (long-term)** — Jekyll/GitHub Pages → owned stack (Vercel/Netlify + Next.js or Astro) with PostHog analytics. Unlocks proper build pipeline, analytics, dynamic features. Re-enable Vercel/PostHog/Supabase MCPs when starting this.

## Decisions Made

- 2026-05-31: Context pruning — disabled 10 MCPs + 97 skills irrelevant to current Jekyll+Node.js stack. Written to `.claude/settings.local.json`. Re-enable design/platform tools when redesign begins.
